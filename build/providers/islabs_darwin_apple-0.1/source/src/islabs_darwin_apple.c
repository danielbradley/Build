...
static bool processCommand( Command* command, const BuildParameters* parameters )
{
	bool status = 1;

	if ( !BuildParameters_isNo( parameters ) )
	{
		if ( BuildParameters_isLog( parameters ) )
		{
			const Global* global = BuildParameters_getGlobal( parameters );
			Command_print( command, ((Global*) global)->logFile );
		}
...
//	fprintf( stdout, "ISLabs::linux-gnu::gnu::compile( %s )\n", Path_getCondensed( Directory_getRealPath( target ) ) );
	for ( i = 0; i < count; i++ )
	{
		...
		//List_copyItem( arguments, "-Wno-long-double" );
...
//
//	Mac OS X uses a form of the GNU Compiler Collection GCC however, the arguments used are
//	different.
//
//	ld -r -S OBJECTS LIBRARIES
//
//	-r	Maintain relocation information
...
//	fprintf( stdout, "ISLabs::linux-gnu::gnu::link()\n" );
	...
	char* target_location_lib_long  = CharString_cat3( target_location_lib, so_name->long_name,  ".1.dylib" );
	char* target_location_lib_short = CharString_cat3( target_location_lib, so_name->short_name, "xx.1.dylib" );
	...
	if ( 0 == CharString_compare( context->linker, "libtool" ) )
	{
		...
		List_copyItem( arguments, "-std=gnu++98" );
		...
		//List_copyItem( arguments, "-arch_only" );
		//List_copyItem( arguments, "ppc" );

		//List_copyItem( arguments, "-lstdc++.6" );
		//List_copyItem( arguments, "-lc" );
		//List_copyItem( arguments, "-lSystem.B" );
...
Provider* GetInstance()
{
	...
	provider->info = "ISLabs Darwin Appple (GNU Compiler Collection)";
	provider->vendor = "ISLabs";
	provider->name = "darwin_apple";
	provider->version = "0.1";
	provider->object = new_ToolSetProvider();	
	return provider;
}

Provider* CreateBuildProvider_islabs_darwin_apple()
{
	return GetInstance();
}
